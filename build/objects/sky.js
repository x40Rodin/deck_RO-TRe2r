...
System.register(['https://cdn.rodin.io/v0.0.1/vendor/three/THREE.GLOBAL', 'https://cdn.rodin.io/v0.0.1/rodinjs/sculpt/THREEObject', 'https://cdn.rodin.io/v0.0.1/rodinjs/scene/SceneManager'], function (_export, _context) {
    ...
    var THREE, THREEObject, SceneManager, scene, skyShader, Sky, SkyObject, sky;
    ...
    return {
        ...
        execute: function () {
            ...
            skyShader = {
                ...
                "}", "float rayleighPhase(float cosTheta)", "{	 ", "return (3.0 / (16.0*pi)) * (1.0 + pow(cosTheta, 2.0));", "//	return (1.0 / (3.0*pi)) * (1.0 + pow(cosTheta, 2.0));", "//	return (3.0 / 4.0) * (1.0 + pow(cosTheta, 2.0));", "}", "vec3 totalMie(vec3 lambda, vec3 K, float T)", "{", "float c = (0.2 * T ) * 10E-18;", "return 0.434 * c * pi * pow((2.0 * pi) / lambda, vec3(v - 2.0)) * K;", "}", "float hgPhase(float cosTheta, float g)", "{", "return (1.0 / (4.0*pi)) * ((1.0 - pow(g, 2.0)) / pow(1.0 - 2.0*g*cosTheta + pow(g, 2.0), 1.5));", "}", "float sunIntensity(float zenithAngleCos)", "{",
            ...
            _export('SkyObject', SkyObject = function (_THREEObject) {
                _inherits(SkyObject, _THREEObject);
                ...
                function SkyObject() {
                    ...
                    var _this = _possibleConstructorReturn(this, (SkyObject.__proto__ || Object.getPrototypeOf(SkyObject)).call(this, sky.mesh));

                    var sunSphere = new THREE.Mesh(new THREE.SphereBufferGeometry(200, 16, 8), new THREE.MeshBasicMaterial({ color: 0xffffff }));
                    ...
                    _this.on("update", function () {
                        ...
                        effectController.azimuth -= 0.00001;
                        var theta = Math.PI * (effectController.inclination - 0.5);
                        var phi = 2 * Math.PI * (effectController.azimuth - 0.5);
            ...
            _export('SkyObject', SkyObject);

            _export('sky', sky = new SkyObject());

            _export('sky', sky);

            sky.on('ready', function (evt) {
                evt.target.object3D.scale.set(.01, .01, .01);
                scene.add(evt.target.object3D);
                scene.add(evt.target.sun);
            });
        }
    };
});