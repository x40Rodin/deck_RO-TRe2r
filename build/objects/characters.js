...
System.register(['https://cdn.rodin.io/v0.0.1/rodinjs/sculpt/ModelLoader'], function (_export, _context) {
    "use strict";
    ...
    function createMan() {
        var person = ModelLoader.load('./models/men/human_men.json');
        person.on('ready', function (evt) {
            var materials = evt.target.object3D.children[0].material.materials;
    ...
    function createWoman() {
        var person = ModelLoader.load('./models/woman/human_women.json');
        person.on('ready', function (evt) {
            var materials = evt.target.object3D.children[0].material.materials;
    ...
    function createMan2() {
        var person = ModelLoader.load('./models/men1/human_men_02.json');
        person.on('ready', function (evt) {
            var materials = evt.target.object3D.children[0].material.materials;
    ...
    function getAngle(a, b) {
        ...
        setters: [function (_httpsCdnRodinIoV001RodinjsSculptModelLoader) {
            ModelLoader = _httpsCdnRodinIoV001RodinjsSculptModelLoader.ModelLoader;
        }],
        execute: function () {
            _export('createMan2', createMan2);

            _export('createMan', createMan);

            _export('createWoman', createWoman);

            _export('getAngle', getAngle);
        }
    };
});